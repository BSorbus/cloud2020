...
def build_users_uke_bi_uke_members
  @uke_bi_array.each do |i|
    Member.create(group: @uke_bi, user_id: i, author_id: 1) #unless Member.find_by(group: @bi, user_id: i) 
    puts "CREATED MEMBER:   #{@uke_bi.name} + " << User.find(i).email
  end  

  @uke_bi_wit_array.each do |i|
    Member.create(group: @uke_bi_wit, user_id: i, author_id: 1) #unless Member.find_by(group: @bi_wit, user_id: i) 
    puts "CREATED MEMBER:   #{@uke_bi_wit.name} + " << User.find(i).email
  end  

  @uke_bi_wur_array.each do |i|
    Member.create(group: @uke_bi_wur, user_id: i, author_id: 1) #unless Member.find_by(group: @bi_wur, user_id: i) 
    puts "CREATED MEMBER:   #{@uke_bi_wur.name} + " << User.find(i).email
  end  

  @uke_bi_wwt_array.each do |i|
    Member.create(group: @uke_bi_wwt, user_id: i, author_id: 1) #unless Member.find_by(group: @bi_wwt, user_id: i) 
